...
class product
{
    ...
    public function getAll($page = 1, $total = 8, $sort = 'asc')
    {
        if ($page <= 0) {
            $page = 1;
        }
        ...
        $query =
            "SELECT products.*, categories.name as cateName
             ...
             order by products.promotionPrice $sort
             LIMIT $tmp, $total";
        $result = $this->db->select($query);
        return $result;
    }
    ...
    public function getSuperSaleProducts()
    {
        $query =
            "SELECT *
			 FROM products
			 WHERE products.status = 1
             order by (originalPrice - promotionPrice) DESC
             LIMIT 8";
        $result = $this->db->select($query);
        return $result;
    }

    public function getProductsByCateId($page = 1, $cateId, $total = 8, $sort = 'asc')
    {
        if ($page <= 0) {
            $page = 1;
        }
    ...
    public function searchProductsByName($searchTerm, $page = 1, $total = 8)
    {
        if ($page <= 0) {
            $page = 1;
        }
        $tmp = ($page - 1) * $total;
        $query = "SELECT * FROM products WHERE name LIKE '%$searchTerm%' AND status = 1 LIMIT $tmp, $total";
        $mysqli_result = $this->db->select($query);
        if ($mysqli_result) {
    ...
    public function getAllProducts($page = 1, $total = 8) {
        return $this->getAll(); // Gọi hàm getAll để lấy tất cả sản phẩm
    }
}
?>